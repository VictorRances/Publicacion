...
class cpersonal_list extends cpersonal {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idper->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idper->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->nombreper->AdvancedSearch->ToJSON(), ","); // Field nombreper
		$sFilterList = ew_Concat($sFilterList, $this->numeroper->AdvancedSearch->ToJSON(), ","); // Field numeroper
		$sFilterList = ew_Concat($sFilterList, $this->correoper->AdvancedSearch->ToJSON(), ","); // Field correoper
		$sFilterList = ew_Concat($sFilterList, $this->contra->AdvancedSearch->ToJSON(), ","); // Field contra
		if ($this->BasicSearch->Keyword <> "") {
	...
	function RestoreFilterList() {
		...
		// Field nombreper
		$this->nombreper->AdvancedSearch->SearchValue = @$filter["x_nombreper"];
		$this->nombreper->AdvancedSearch->SearchOperator = @$filter["z_nombreper"];
		$this->nombreper->AdvancedSearch->SearchCondition = @$filter["v_nombreper"];
		$this->nombreper->AdvancedSearch->SearchValue2 = @$filter["y_nombreper"];
		$this->nombreper->AdvancedSearch->SearchOperator2 = @$filter["w_nombreper"];
		$this->nombreper->AdvancedSearch->Save();

		// Field numeroper
		$this->numeroper->AdvancedSearch->SearchValue = @$filter["x_numeroper"];
		$this->numeroper->AdvancedSearch->SearchOperator = @$filter["z_numeroper"];
		$this->numeroper->AdvancedSearch->SearchCondition = @$filter["v_numeroper"];
		$this->numeroper->AdvancedSearch->SearchValue2 = @$filter["y_numeroper"];
		$this->numeroper->AdvancedSearch->SearchOperator2 = @$filter["w_numeroper"];
		$this->numeroper->AdvancedSearch->Save();

		// Field correoper
		$this->correoper->AdvancedSearch->SearchValue = @$filter["x_correoper"];
		$this->correoper->AdvancedSearch->SearchOperator = @$filter["z_correoper"];
		$this->correoper->AdvancedSearch->SearchCondition = @$filter["v_correoper"];
		$this->correoper->AdvancedSearch->SearchValue2 = @$filter["y_correoper"];
		$this->correoper->AdvancedSearch->SearchOperator2 = @$filter["w_correoper"];
		$this->correoper->AdvancedSearch->Save();

		// Field contra
		$this->contra->AdvancedSearch->SearchValue = @$filter["x_contra"];
		$this->contra->AdvancedSearch->SearchOperator = @$filter["z_contra"];
		$this->contra->AdvancedSearch->SearchCondition = @$filter["v_contra"];
		$this->contra->AdvancedSearch->SearchValue2 = @$filter["y_contra"];
		$this->contra->AdvancedSearch->SearchOperator2 = @$filter["w_contra"];
		$this->contra->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->nombreper, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->numeroper, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->correoper, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->contra, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->nombreper); // nombreper
			$this->UpdateSort($this->numeroper); // numeroper
			$this->UpdateSort($this->correoper); // correoper
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->setSessionOrderBy($sOrderBy);
				$this->idper->setSort("");
				$this->nombreper->setSort("");
				$this->numeroper->setSort("");
				$this->correoper->setSort("");
				$this->contra->setSort("");
			}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idper->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpersonallistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
fpersonallist.ValidateRequired = true;
<?php } else { ?>
fpersonallist.ValidateRequired = false; 
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) {
		if ($personal_list->TotalRecs <= 0)
			$personal_list->TotalRecs = $personal->SelectRecordCount();
	} else {
	...
	if ($personal->CurrentAction == "" && $personal_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$personal_list->setWarningMessage(ew_DeniedMsg());
...
<form name="fpersonallistsrch" id="fpersonallistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
...
<div class="panel panel-default ewGrid personal">
...
<?php } ?>
<input type="hidden" name="t" value="personal">
<div id="gmp_personal" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($personal_list->TotalRecs > 0) { ?>
<table id="tbl_personallist" class="table ewTable">
<?php echo $personal->TableCustomInnerHtml ?>
<thead><!-- Table header -->
...
<?php if ($personal->idper->Visible) { // idper ?>
	<?php if ($personal->SortUrl($personal->idper) == "") { ?>
		<th data-name="idper"><div id="elh_personal_idper" class="personal_idper"><div class="ewTableHeaderCaption"><?php echo $personal->idper->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="idper"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $personal->SortUrl($personal->idper) ?>',1);"><div id="elh_personal_idper" class="personal_idper">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $personal->idper->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($personal->idper->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($personal->idper->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($personal->nombreper->Visible) { // nombreper ?>
	<?php if ($personal->SortUrl($personal->nombreper) == "") { ?>
		<th data-name="nombreper"><div id="elh_personal_nombreper" class="personal_nombreper"><div class="ewTableHeaderCaption"><?php echo $personal->nombreper->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nombreper"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $personal->SortUrl($personal->nombreper) ?>',1);"><div id="elh_personal_nombreper" class="personal_nombreper">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $personal->nombreper->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($personal->nombreper->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($personal->nombreper->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($personal->numeroper->Visible) { // numeroper ?>
	<?php if ($personal->SortUrl($personal->numeroper) == "") { ?>
		<th data-name="numeroper"><div id="elh_personal_numeroper" class="personal_numeroper"><div class="ewTableHeaderCaption"><?php echo $personal->numeroper->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="numeroper"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $personal->SortUrl($personal->numeroper) ?>',1);"><div id="elh_personal_numeroper" class="personal_numeroper">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $personal->numeroper->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($personal->numeroper->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($personal->numeroper->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($personal->correoper->Visible) { // correoper ?>
	<?php if ($personal->SortUrl($personal->correoper) == "") { ?>
		<th data-name="correoper"><div id="elh_personal_correoper" class="personal_correoper"><div class="ewTableHeaderCaption"><?php echo $personal->correoper->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="correoper"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $personal->SortUrl($personal->correoper) ?>',1);"><div id="elh_personal_correoper" class="personal_correoper">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $personal->correoper->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($personal->correoper->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($personal->correoper->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($personal->contra->Visible) { // contra ?>
	<?php if ($personal->SortUrl($personal->contra) == "") { ?>
		<th data-name="contra"><div id="elh_personal_contra" class="personal_contra"><div class="ewTableHeaderCaption"><?php echo $personal->contra->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="contra"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $personal->SortUrl($personal->contra) ?>',1);"><div id="elh_personal_contra" class="personal_contra">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $personal->contra->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($personal->contra->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($personal->contra->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($personal->idper->Visible) { // idper ?>
		<td data-name="idper"<?php echo $personal->idper->CellAttributes() ?>>
<span id="el<?php echo $personal_list->RowCnt ?>_personal_idper" class="personal_idper">
<span<?php echo $personal->idper->ViewAttributes() ?>>
<?php echo $personal->idper->ListViewValue() ?></span>
</span>
<a id="<?php echo $personal_list->PageObjName . "_row_" . $personal_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($personal->nombreper->Visible) { // nombreper ?>
		<td data-name="nombreper"<?php echo $personal->nombreper->CellAttributes() ?>>
<span id="el<?php echo $personal_list->RowCnt ?>_personal_nombreper" class="personal_nombreper">
<span<?php echo $personal->nombreper->ViewAttributes() ?>>
<?php echo $personal->nombreper->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($personal->numeroper->Visible) { // numeroper ?>
		<td data-name="numeroper"<?php echo $personal->numeroper->CellAttributes() ?>>
<span id="el<?php echo $personal_list->RowCnt ?>_personal_numeroper" class="personal_numeroper">
<span<?php echo $personal->numeroper->ViewAttributes() ?>>
<?php echo $personal->numeroper->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($personal->correoper->Visible) { // correoper ?>
		<td data-name="correoper"<?php echo $personal->correoper->CellAttributes() ?>>
<span id="el<?php echo $personal_list->RowCnt ?>_personal_correoper" class="personal_correoper">
<span<?php echo $personal->correoper->ViewAttributes() ?>>
<?php echo $personal->correoper->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($personal->contra->Visible) { // contra ?>
		<td data-name="contra"<?php echo $personal->contra->CellAttributes() ?>>
<span id="el<?php echo $personal_list->RowCnt ?>_personal_contra" class="personal_contra">
<span<?php echo $personal->contra->ViewAttributes() ?>>
<?php echo $personal->contra->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
<?php if (!isset($personal_list->Pager)) $personal_list->Pager = new cPrevNextPager($personal_list->StartRec, $personal_list->DisplayRecs, $personal_list->TotalRecs) ?>
