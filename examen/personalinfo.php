...
class cpersonal extends cTable {
	...
	function __construct() {
		...
		// idper
		$this->idper = new cField('personal', 'personal', 'x_idper', 'idper', '`idper`', '`idper`', 3, -1, FALSE, '`idper`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// nombreper
		$this->nombreper = new cField('personal', 'personal', 'x_nombreper', 'nombreper', '`nombreper`', '`nombreper`', 201, -1, FALSE, '`nombreper`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->nombreper->Sortable = TRUE; // Allow sort
		$this->fields['nombreper'] = &$this->nombreper;

		// numeroper
		$this->numeroper = new cField('personal', 'personal', 'x_numeroper', 'numeroper', '`numeroper`', '`numeroper`', 201, -1, FALSE, '`numeroper`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->numeroper->Sortable = TRUE; // Allow sort
		$this->fields['numeroper'] = &$this->numeroper;

		// correoper
		$this->correoper = new cField('personal', 'personal', 'x_correoper', 'correoper', '`correoper`', '`correoper`', 201, -1, FALSE, '`correoper`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->correoper->Sortable = TRUE; // Allow sort
		$this->fields['correoper'] = &$this->correoper;

		// contra
		$this->contra = new cField('personal', 'personal', 'x_contra', 'contra', '`contra`', '`contra`', 201, -1, FALSE, '`contra`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->contra->Sortable = TRUE; // Allow sort
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('idper', $rs))
				ew_AddFilter($where, ew_QuotedName('idper', $this->DBID) . '=' . ew_QuotedValue($rs['idper'], $this->idper->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idper->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idper@", ew_AdjustSql($this->idper->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "idper:" . ew_VarToJson($this->idper->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idper->CurrentValue)) {
			$sUrl .= "idper=" . urlencode($this->idper->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["idper"]))
				$arKeys[] = ew_StripSlashes($_POST["idper"]);
			elseif (isset($_GET["idper"]))
				$arKeys[] = ew_StripSlashes($_GET["idper"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->nombreper->EditValue = $this->nombreper->CurrentValue;
		...
		$this->numeroper->EditValue = $this->numeroper->CurrentValue;
		...
		$this->correoper->EditValue = $this->correoper->CurrentValue;
		...
		$this->contra->EditValue = $this->contra->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nombreper->Exportable) $Doc->ExportCaption($this->nombreper);
					if ($this->numeroper->Exportable) $Doc->ExportCaption($this->numeroper);
					if ($this->correoper->Exportable) $Doc->ExportCaption($this->correoper);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->nombreper->Exportable) $Doc->ExportField($this->nombreper);
						if ($this->numeroper->Exportable) $Doc->ExportField($this->numeroper);
						if ($this->correoper->Exportable) $Doc->ExportField($this->correoper);
