...
function CurrentProjectID() {
	...
	return "{33D340AA-42AB-4291-828D-AC1DA9E53193}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$validateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$validateUser = $customValidateUser || ew_ComparePassword($rs->fields('contra'), $pwd, $encrypted);
					if ($validateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('correoper')); // Load user name
